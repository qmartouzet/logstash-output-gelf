...
| <<plugins-{type}s-{plugin}-metadata_nested_field_separator>> |<<string,string>>|No
...
[id="plugins-{type}s-{plugin}-metadata_nested_field_separator"]
===== `metadata_nested_field_separator`

  * Value type is <<string,string>>
  * Default value is `"_"`

When `ship_metadata` is set, specify key separator for nested fields. For example, `"foo" => { "bar" => "baz", "qux" => { "quux" => "corge"} }` gives 2 fields : `_foo_bar = baz` and `_foo_qux_quux = corge`.
